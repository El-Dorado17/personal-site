...
export function Projects() {
    ...
    const prevSlideSOL = () =>{
        const isFirstSlide = currentIndex === 0;
        const newIndex = isFirstSlide ? imagesSOL.length -1 : currentIndex -1
        setCurrentIndex(newIndex)
    }

    const nextSlideSOL = () =>{
        const isLastSlide = currentIndex === imagesSOL.length -1
        const newIndex = isLastSlide ? 0 : currentIndex + 1
        setCurrentIndex(newIndex)
    }
...
{/* Left Button */}
        <button onClick={prevSlideQK} type="button" className="hidden group-hover:block absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
...
{/* Right Button */}
        <button onClick={nextSlideQK} type="button" className="hidden group-hover:block absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next> 
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                <svg className="w-4 h-4 text-white dark:text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
